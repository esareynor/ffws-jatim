...
@section('title', 'Add Device Configuration')
...
<div class="container-fluid px-4 py-6">
    ...
    <div class="mb-6">
        <div class="flex items-center mb-2">
            ...
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Add Device Configuration</h1>
        </div>
        <p class="text-sm text-gray-600 dark:text-gray-400">Configure extended device information, location, and maintenance schedule</p>
    </div>

    <!-- Form -->
    <form action="{{ route('admin.device-values.store') }}" method="POST" class="max-w-4xl">
        @csrf
                ...
                <div class="md:col-span-2">
                    ...
                    <select name="mas_device_code" required
                        ...
                        @foreach($devices as $device)
                            <option value="{{ $device->code }}" {{ old('mas_device_code') == $device->code ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <select name="mas_device_parameter_code"
                        ...
                        @foreach($deviceParameters as $param)
                            <option value="{{ $param->code }}" {{ old('mas_device_parameter_code') == $param->code ? 'selected' : '' }}>
                                {{ $param->name }}
                            </option>
                        @endforeach
                    </select>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6 mb-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div>
                    ...
                    <input type="number" step="0.01" name="elevation" value="{{ old('elevation') }}"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="mas_river_basin_code"
                        ...
                        @foreach($riverBasins as $basin)
                            <option value="{{ $basin->code }}" {{ old('mas_river_basin_code') == $basin->code ? 'selected' : '' }}>
                                {{ $basin->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="mas_watershed_code"
                        ...
                        @foreach($watersheds as $watershed)
                            <option value="{{ $watershed->code }}" {{ old('mas_watershed_code') == $watershed->code ? 'selected' : '' }}>
                                {{ $watershed->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="mas_city_code"
                        ...
                        @foreach($cities as $city)
                            <option value="{{ $city->code }}" {{ old('mas_city_code') == $city->code ? 'selected' : '' }}>
                                {{ $city->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="mas_regency_code"
                        ...
                        @foreach($regencies as $regency)
                            <option value="{{ $regency->code }}" {{ old('mas_regency_code') == $regency->code ? 'selected' : '' }}>
                                {{ $regency->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="mas_village_code"
                        ...
                        @foreach($villages as $village)
                            <option value="{{ $village->code }}" {{ old('mas_village_code') == $village->code ? 'selected' : '' }}>
                                {{ $village->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="mas_upt_code"
                        ...
                        @foreach($upts as $upt)
                            <option value="{{ $upt->code }}" {{ old('mas_upt_code') == $upt->code ? 'selected' : '' }}>
                                {{ $upt->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="mas_uptd_code"
                        ...
                        @foreach($uptds as $uptd)
                            <option value="{{ $uptd->code }}" {{ old('mas_uptd_code') == $uptd->code ? 'selected' : '' }}>
                                {{ $uptd->name }}
                            </option>
                        @endforeach
                    </select>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6 mb-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div>
                    ...
                    <input type="date" name="next_maintenance" value="{{ old('next_maintenance') }}"
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6 mb-6">
            ...
            <div>
                ...
                <textarea name="description" rows="4"
                    ...
                    class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500">{{ old('description') }}</textarea>
            </div>
        </div>
