...
const GoogleMapsSearchbar = ({
  ...
  const suggestions = useMemo(() => {
    if (!searchValue.trim() || isLoadingDevices) {
      return [];
    }
    ...
    const deviceSuggestions = devicesData
      .filter(device => device.name && normalizeString(device.name).includes(normalizedInput))
      .map(device => device.name)
      .slice(0, 8); // Tampilkan hingga 8 hasil device

    return deviceSuggestions;
  }, [searchValue, devicesData, isLoadingDevices]);
  ...
  const performSearch = useCallback(
    (query) => {
      ...
      let foundDevice = devicesData.find(
        (device) => device.name && normalizeString(device.name) === normalizedQuery
      );

      // Jika tidak ada exact match, coba partial match
      if (!foundDevice) {
        foundDevice = devicesData.find((device) =>
          device.name && normalizeString(device.name).includes(normalizedQuery)
        );
      }
  ...
  return (
    <div
      ...
      style={{
        right: isSidebarOpen ? undefined : '3.5rem',
          ...
          {isFocused && suggestions.length > 0 && (
            <div className="absolute top-full left-0 right-0 mt-1 bg-white rounded-lg shadow-lg z-10 overflow-hidden max-h-60 overflow-y-auto">
              {suggestions.map((suggestion, index) => (
                <div
                  key={index}
                  className="px-3 sm:px-4 py-2 hover:bg-gray-100 cursor-pointer transition-colors text-xs sm:text-sm flex items-center"
                  onClick={() => handleSuggestionClick(suggestion)}
                >
                  <svg
                    className="w-3 h-3 sm:w-4 sm:h-4 text-gray-400 mr-2 shrink-0"
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                  >
                  ...
                  <span className="truncate">{suggestion.charAt(0).toUpperCase() + suggestion.slice(1)}</span>
                </div>
              ))}
            </div>
          )}
