@extends('layouts.admin')

@section('title', 'Desa/Kelurahan')
@section('page-title', 'Desa/Kelurahan')
@section('page-description', 'Kelola data desa dan kelurahan')
@section('breadcrumb', 'Desa/Kelurahan')

@section('content')
<div class="space-y-6" x-data="villageData()" x-init="init()">

    {{-- Filter Section --}}
    @php
        $filterConfig = [
            [
                'type' => 'text',
                'name' => 'search',
                'label' => 'Cari Desa/Kelurahan',
                'placeholder' => 'Cari berdasarkan kode atau nama...'
            ],
            [
                'type' => 'select',
                'name' => 'regency_code',
                'label' => 'Kecamatan',
                'empty_option' => 'Semua Kecamatan',
                'options' => $regencies->map(function($regency) {
                    return ['value' => $regency->regencies_code, 'label' => $regency->regencies_name . ' (' . ($regency->city->name ?? '-') . ')'];
                })->toArray()
            ],
            [
                'type' => 'select',
                'name' => 'per_page',
                'label' => 'Per Halaman',
                'options' => [
                    ['value' => '10', 'label' => '10'],
                    ['value' => '15', 'label' => '15'],
                    ['value' => '25', 'label' => '25'],
                    ['value' => '50', 'label' => '50']
                ]
            ]
        ];
    @endphp

    <x-filter-bar 
        title="Filter & Pencarian Desa/Kelurahan"
        :filters="$filterConfig"
        :action="route('admin.region.villages.index')"
        gridCols="md:grid-cols-3"
    />

    {{-- Villages Table --}}
    <x-table
        title="Daftar Desa/Kelurahan"
        :headers="$tableHeaders"
        :rows="$villages"
        searchable
        searchPlaceholder="Cari desa/kelurahan..."
        :pagination="$villages->links()"
    >
        <x-slot:actions>
            <x-admin.button type="button" variant="primary" @click="openCreateModal()">
                <i class="fa-solid fa-plus -ml-1 mr-2"></i>
                Tambah Desa/Kelurahan
            </x-admin.button>
        </x-slot:actions>
    </x-table>

    {{-- Modal Create/Edit --}}
    <x-admin.modal :show="false" name="village-modal" size="md" :close-on-backdrop="true">
        <x-slot:title>
            <span x-text="isEdit ? 'Edit Desa/Kelurahan' : 'Tambah Desa/Kelurahan'"></span>
        </x-slot:title>

        <form @submit.prevent="submitForm()" class="space-y-6">
            <x-admin.form-input
                type="select"
                name="regencies_code"
                label="Kecamatan"
                x-model="formData.regencies_code"
                required="true"
                :options="$regencies->map(function($regency) {
                    return ['value' => $regency->regencies_code, 'label' => $regency->regencies_name . ' (' . ($regency->city->name ?? '-') . ')'];
                })->prepend(['value' => '', 'label' => 'Pilih Kecamatan'])->toArray()"
            />
            <x-admin.form-input
                type="text"
                name="code"
                label="Kode Desa/Kelurahan"
                x-model="formData.code"
                placeholder="Contoh: DESA-001"
                required="true"
            />
            <x-admin.form-input
                type="text"
                name="name"
                label="Nama Desa/Kelurahan"
                x-model="formData.name"
                placeholder="Contoh: Kedungdoro"
                required="true"
            />
        </form>

        <x-slot:footer>
            <x-admin.button variant="outline" @click="closeModal()">
                Batal
            </x-admin.button>
            <x-admin.button variant="primary" @click="submitForm()">
                <span x-text="isEdit ? 'Update' : 'Simpan'"></span>
            </x-admin.button>
        </x-slot:footer>
    </x-admin.modal>
</div>

@push('scripts')
<script>
function villageData() {
    return {
        showModal: false,
        isEdit: false,
        formData: {
            id: null,
            code: '',
            name: '',
            regencies_code: ''
        },

        init() {
            window.addEventListener('open-edit-village', (e) => {
                const item = e.detail || {};
                this.openEditModal(item);
            });
        },

        openCreateModal() {
            this.isEdit = false;
            this.resetForm();
            this.$dispatch('open-modal', 'village-modal');
        },

        openEditModal(village) {
            this.isEdit = true;
            this.formData = {
                id: village.id,
                code: village.code,
                name: village.name,
                regencies_code: village.regencies_code
            };
            this.$dispatch('open-modal', 'village-modal');
        },

        closeModal() {
            this.$dispatch('close-modal', 'village-modal');
            this.resetForm();
        },

        resetForm() {
            this.formData = {
                id: null,
                code: '',
                name: '',
                regencies_code: ''
            };
        },

        async submitForm() {
            try {
                const url = this.isEdit
                    ? `/admin/region/villages/${this.formData.id}`
                    : '/admin/region/villages';

                const method = this.isEdit ? 'PUT' : 'POST';

                const response = await fetch(url, {
                    method: method,
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
                    },
                    body: JSON.stringify(this.formData)
                });

                const data = await response.json();

                if (response.ok) {
                    window.location.reload();
                } else {
                    alert(data.message || 'Terjadi kesalahan');
                }
            } catch (error) {
                console.error('Error:', error);
                alert('Terjadi kesalahan saat menyimpan data');
            }
        }
    };
}
</script>
@endpush
@endsection
